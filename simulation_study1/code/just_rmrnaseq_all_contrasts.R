...
for(row_id in 1:4){
  ...
  sim_results <- pblapply(1:n_sim, function(j){
    ...
    res_rmrnaseq = TC_CAR1(counts = cnts, 
                           design = dmat, 
                           Subject = as.factor(smd$id), 
                           Time = smd$time, 
                           C.matrix = Cmat,
                           Nboot = 100,
                           ncores = 1, # change this to higher value if possible, method takes a long time to run 
                           print.progress = T)
  ...
  file_out <- paste0("just_rmrnaseq_ROC_MIX_n_", n_sample_per_group, ".RDS")
