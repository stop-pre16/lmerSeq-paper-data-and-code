...
for(row_id in 1:4){
  ...
  sim_results <- pblapply(1:n_sim, function(j){
    ...
    expr_dream = voomWithDreamWeights(counts = cnts, 
                                      formula = ~ group * time + (time|subject),
                                      data = smd,
                                      BPPARAM = serialParam)
    fit.lmerSeq.continuous_ri_rs <- lmerSeq.fit(form = ~ group*time + (time|subject),
    ...
    cont_ri_rs_ineraction.ttest <- lmerSeq.contrast(lmerSeq_results = fit.lmerSeq.continuous_ri_rs,
    ...
    cont_ri_rs_time.ttest <- lmerSeq.contrast(lmerSeq_results = fit.lmerSeq.continuous_ri_rs,
    ...
    cont_ri_rs_group.ttest <- lmerSeq.contrast(lmerSeq_results = fit.lmerSeq.continuous_ri_rs,
                                               contrast_mat = group.ttest.cont,
                                               p_adj_method = 'BH',
                                               ddf = 'Satterthwaite',
                                               sort_results = F, include_singular=T)
    rm(fit.lmerSeq.continuous_ri_rs)
    gc()
    gc()
    
    cont_ri_rs_tab_out_int = data.frame(sim_data$param) %>% 
      mutate(gene = as.factor(1:nrow(sim_data$param))) %>% 
      mutate(true_de = beta_ints != 0) %>% 
      left_join(cont_ri_rs_ineraction.ttest$summary_table %>% dplyr::select(-lower, -upper)) %>% 
      mutate(sim_num = j, n_per_grp = n_sample_per_group, contrast = 'Interaction', fit = "cont_ri_rs")
    
    cont_ri_rs_tab_out_btw = data.frame(sim_data$param) %>% 
      mutate(gene = as.factor(1:nrow(sim_data$param))) %>% 
      mutate(true_de = beta_ints != 0) %>% 
      left_join(cont_ri_rs_group.ttest$summary_table %>% dplyr::select(-lower, -upper)) %>% 
      mutate(sim_num = j, n_per_grp = n_sample_per_group, contrast = 'Between', fit = "cont_ri_rs")
    
    cont_ri_rs_tab_out_wtn = data.frame(sim_data$param) %>% 
      mutate(gene = as.factor(1:nrow(sim_data$param))) %>% 
      mutate(true_de = beta_ints != 0) %>% 
      left_join(cont_ri_rs_time.ttest$summary_table %>% dplyr::select(-lower, -upper)) %>% 
      mutate(sim_num = j, n_per_grp = n_sample_per_group, contrast = 'Within', fit = "cont_ri_rs")
    
    out_tab_vst = rbind(cont_ri_rs_tab_out_int, cont_ri_rs_tab_out_btw, cont_ri_rs_tab_out_wtn)
  ...
  file_out <- paste0("DREAM_4tp_cont_ri_rs_n_", n_sample_per_group, ".RDS")
