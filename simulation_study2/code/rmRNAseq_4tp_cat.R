...
for(row_id in 1:4){
  ...
  sim_results <- pblapply(1:n_sim, function(j){
    ...
    dmat = model.matrix(~ smd$group * smd$time_factor)
    
    Cmat = list()
    Cmat[[1]] = cbind(c(0,0,0,0,0,0,0,1))
    Cmat[[2]] = cbind(c(0,0,0,0,1,0,0,1))
    Cmat[[3]] = cbind(c(0,1,0,0,0,0,0,1))

    names(Cmat) = c("Interaction", "Between", "Within")
    ...
    out_tab_all = rbind(tab_out_int, tab_out_btw, tab_out_wtn)
  ...
  file_out <- paste0("just_rmrnaseq_4tp_cat_n_", n_sample_per_group, ".RDS")
